...
function NewsOfDayPage() {
  ...
  useEffect(() => {
    axios.get(`${API_BASE}/api/list/`)
      .then(res => {
        const data = res.data;
        if (data.allnewsofday) {
          setNewsList(data.allnewsofday);
        } else if (data.twonewsofday) {
          setNewsList(data.twonewsofday);
        }
  ...
  const handleSearchSubmit = (e) => {
    ...
    setLoading(true);
    axios.get(`${API_BASE}/api/newsoftheday/?search=${encodeURIComponent(searchQuery)}`)
      .then(res => {
              ...
              newsList.map((item, index) => (
                <div key={index} className="nod-news-card" onClick={() => registerView('newsoftheday', item.slug)}>
                  <div className="nod-card-media">
                    ...
                    />
                    <span className="nod-card-badge nod-card-badge-newsoftheday">اخبار روز</span>
                  </div>
                  <div className="nod-card-content">
                  <p className="nod-card-summary">{item.txt_news}</p>
                    <h2 className="nod-card-heading">{item.title_news || item.txt_news}</h2>
