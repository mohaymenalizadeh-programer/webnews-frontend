...
function LifestylePage() {
  ...
  useEffect(() => {
    axios.get(`${API_BASE}/api/list/`)
      .then(res => {
        const data = res.data;
        if (data.alllifestyle) setNewsList(data.alllifestyle);
  ...
  const handleSearchSubmit = (e) => {
    ...
    setLoading(true);
    axios.get(`${API_BASE}/api/lifestyle/?search=${encodeURIComponent(searchQuery)}`)
      .then(res => {
          ...
          <div className="nod-grid-right">
            {loading ? (
              ...
              newsList.map((item, index) => (
                <div key={index} className="nod-news-card" onClick={() => registerView('lifestyle', item.slug)}>
                  <div className="nod-card-media">
                    <img 
                      src={getImgUrl(item.img_Lifestyle)} 
                      alt={item.longtitle || "سبک زندگی"} 
                      onError={(e) => { e.target.src = 'https://via.placeholder.com/300x180?text=Error'; }}
                    />
                    <span className="nod-card-badge nod-card-badge-lifestyle">سبک زندگی</span>
                  </div>
                  <div className="nod-card-content">
                  <h2 className="nod-card-summary">{item.begtxt}</h2>
                    <p className="nod-card-heading">{item.longtitle}</p>
