...
function TechnologyPage() {
  ...
  useEffect(() => {
    axios.get(`${API_BASE}/api/list/`)
      .then(res => {
        const data = res.data;
        if (data.alltechnology) setNewsList(data.alltechnology);
  ...
  const handleSearchSubmit = (e) => {
    ...
    setLoading(true);
    axios.get(`${API_BASE}/api/technology/?search=${encodeURIComponent(searchQuery)}`)
      .then(res => {
      ...
      >
        ...
        />
              ...
              newsList.map((item, index) => (
                <div key={index} className="nod-news-card" onClick={() => registerView('technology', item.slug)}>
                  <div className="nod-card-media">
                    <img 
                      src={getImgUrl(item.img_Technology)} 
                      alt={item.explanation || "تکنولوژی"} 
                      onError={(e) => { e.target.src = 'https://via.placeholder.com/300x180?text=Error'; }}
                    />

                    <span className="nod-card-badge nod-card-badge-technology">تکنولوژی</span>
                  </div>
                  <div className="nod-card-content">
                    <p className="nod-card-summary">{item.matn}</p>
                    <h2 className="nod-card-heading">{item.explanation || item.page_newsTechnology_title}</h2>
